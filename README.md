...
一个面向 Windows 桌面的 VSLLM 自动签到、自动今日任务、自动翻牌抽奖和守护工具。

项目针对 `https://vsllm.com/console/personal` 的日常操作做了一个可视化控制台：第一次登录后保存本机登录态，之后可以在后台通过接口执行每日签到、今日任务、翻牌抽奖、读取余额，并在窗口日志里看到每次执行时间、抽奖结果、累计次数和下一轮预计时间。
...
- 支持自动今日任务：达标后领取模型任务奖励、按服务端时长完成站内广告任务、通过本地题库学习自动答题。
...
3. 确认进入个人中心后，关闭浏览器窗口。
4. 回到登录命令窗口按 `Enter`。
5. 回到控制台，点击 `签到+任务+翻牌一次` 或 `开始守护`。

之后使用：

- 想立即执行一次，点击 `签到+任务+翻牌一次`。
- 想自动等待刷新机会，点击 `开始守护`。
- 想停止自动守护，点击 `停止守护`。
- 想只刷新余额，点击 `刷新余额`。
...
| `签到+任务+翻牌一次` | 执行签到、今日任务检查，再按当前剩余机会翻牌。 |
...
2. 检查每日签到入口。
3. 自动检查今日任务（达标领奖、每日答题、站内广告奖励）。
4. 读取当前翻牌次数，例如 `1/3 次`。
5. 读取冷却倒计时，例如 `1 小时 59 分后可再抽`。
6. 调用分享加成接口。
7. 调用翻牌接口。
8. 写入抽奖结果和下一轮时间。
...
| `logs/daily-task-answers.json` | 每日答题本地学习缓存，不包含登录凭据。 |
...
| `自动今日任务` | 开启 | 自动领奖、答题和按正常等待时长完成站内广告任务。 |
...
| `VSLLM_DAILY_TASKS` | 是否自动执行今日任务；`0`/`false` 可关闭，默认开启。 |
| `VSLLM_TASK_AD_LIMIT` | 单轮最多完成几次广告任务，默认 `3`。 |
