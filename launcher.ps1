...
function Set-RunningState {
  ...
  if ($IsRunning) {
    $script:StatusLabel.Text = '状态：正在执行签到+翻牌'
...
function Show-RecentDrawHistory {
  ...
  if ($lines.Count -eq 0) {
    Append-Log '抽奖记录：还没有历史记录。点击[签到+翻牌一次]或[开始守护]后会显示在这里。'
    Update-HistorySummary
    return
  }
...
function Invoke-WatchCheck {
  ...
  $script:WatchNextRunAt = $null
  $script:WatchRound += 1
  Append-Log ("守护：第 {0} 轮开始，执行签到+翻牌。" -f $script:WatchRound)
...
function Start-WatchMode {
  ...
  $script:WatchRound = 0
  $script:WatchNextRunAt = $null
  Append-Log ("守护已启动：每轮会先签到，再翻牌；读不到网页冷却时，每 {0} 分钟兜底检查一次。" -f [int]$script:IntervalBox.Value)
  Invoke-WatchCheck
}
...
$apiOnceMenuItem = New-Object System.Windows.Forms.ToolStripMenuItem('签到+翻牌一次')
...
$script:RunButton.Add_Click({ Invoke-NpmScript 'api' '签到+翻牌一次' })
